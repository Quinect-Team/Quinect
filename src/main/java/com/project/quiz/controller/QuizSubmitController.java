...
public class QuizSubmitController {
	...
	public ResponseEntity<?> submit(@PathVariable("quizId") Long quizId, @RequestBody QuizSubmitRequest request,
		...
		// 6. 총점(만점) 계산
		int maxScore = submission.getQuiz().getQuestions().stream()
				.mapToInt(q -> q.getPoint() != null ? q.getPoint() : 0).sum();

		boolean scorePublic = submission.getQuiz().isScorePublic();

		// 7. 결과 반환
		return ResponseEntity.ok(
				Map.of("submissionId", submissionId, "score", score, "maxScore", maxScore, "scorePublic", scorePublic));
	}
}
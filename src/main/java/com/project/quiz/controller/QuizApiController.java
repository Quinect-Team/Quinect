...
public class QuizApiController {
	...
	public List<QuizDto.ListResponse> getQuizList(@RequestParam(name = "sort", defaultValue = "latest") String sort,
		...
		return quizService.getQuizList(sort, keyword).stream().map(QuizDto.ListResponse::fromEntity).toList();
	}

	@GetMapping("/api/my")
	public List<QuizDto.ListResponse> myQuizzes() {
		return quizService.findMyQuizzes().stream().map(QuizDto.ListResponse::fromEntity).toList();
	}
